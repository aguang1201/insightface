...
def get_embedding(args, imgrec, id, image_size, model):
    ...
    ocontents = []
    for idx in list(range(int(header.label[0]), int(header.label[1]))):
        # print('idx', idx)
...
def main(args):
    ...
    if len(args.model) > 0:
        ...
        if len(cvd) > 0:
            for i in list(range(len(cvd.split(',')))):
                ctx.append(mx.gpu(i))
        if len(ctx) == 0:
            ctx = [mx.cpu()]
            print('use cpu')
    ...
    all_id_list = []
    test_limit = 0
    for ds_id in list(range(len(rec_list))):
        id_list = []
        ...
        if ds_id == 0 or model is None:
            ...
            for id_item in all_id_list:
                X.append(id_item[2])
            X = np.array(X)
            for i in list(range(len(id_list))):
                id_item = id_list[i]
                y = id_item[2]
                sim = np.dot(X, y.T)
    ...
    if len(args.exclude) > 0:
        if os.path.isdir(args.exclude):
            ...
            for i in list(range(data_set.shape[0])):
                data[0] = data_set[i]
                db = mx.io.DataBatch(data=(data,))
                model.forward(db, is_train=False)
                net_out = model.get_outputs()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--include',
                        default='/media/ys1/YS1_Internal_Storage_4T/dataset/face_dataset/iccv19_mtcnn_112_00,/media/ys1/YS1_Internal_Storage_4T/dataset/face_dataset/iccv19_mtcnn_112_01',
                        type=str, help='')
    parser.add_argument('--exclude', default='', type=str, help='')
    parser.add_argument('--output', default='/media/ys1/YS1_Internal_Storage_4T/dataset/face_dataset/dataset_merged', type=str, help='')
    # parser.add_argument('--model', default='../model/softmax,50', help='path to load model.')
    parser.add_argument('--model', default='', help='path to load model.')
    parser.add_argument('--batch-size', default=32, type=int, help='')
    parser.add_argument('--param1', default=0.0, type=float, help='')
